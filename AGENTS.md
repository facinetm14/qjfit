...
| Adding an HTTP route to trigger the job pool refresh                          | The refresh is cron-only, wired in `composition-root/bootstrap.ts` via `node-cron` — no route calls the connectors |
