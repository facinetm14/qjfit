...
QJFit is a stateless, no-signup web application. Job offers are aggregated into a shared pool, refreshed on demand from inside a match request, scoped and gated by the visitor's CV — **France Travail is the only active connector today.** A **Welcome to the Jungle RSS connector is implemented** (`wttj-rss.connector.ts`) but not yet wired into the container (`bind-connectors.ts` has it commented out, pending a real feed URL to test against). Adzuna, JSearch, and HelloWork are planned v1 sources not yet built (PRD §3.2). A visitor uploads a CV and gets it scored against the current pool using an LLM, with results shown in a Vue.js results view. There are no accounts, no sessions, and nothing is persisted beyond the shared job pool itself — see `docs/prd/prd-v1.md` (v3.0), `docs/adr/0015-anonymous-stateless-mvp.md` (the product pivot), `docs/adr/0016-anonymous-stateless-schema-and-runtime-migration.md` (the concrete schema/route/Redis migration), `docs/adr/0021-cv-scoped-on-demand-job-fetching.md` (replacing the cron-driven refresh with per-query-signature on-demand fetching), and `docs/adr/0022-domain-first-backend-restructure.md` (the backend's domain-first directory layout) for the model.

**ADR 0016 migration status.** The pre-pivot surface it supersedes (`GET/PUT /api/profile`, `POST /api/fetch`, the `Profile`/`Score` Prisma models, `Job.status`/`JobStatus`) is fully deleted. §3's cron-driven job pool refresh is superseded by ADR 0021: `EnsureFreshJobPoolForSignatureUseCase` (`fetch-runs/application/usecases/`) is invoked from inside `CreateMatchRequestUseCase`'s fire-and-forget pipeline, scoped by a query signature (`fetch-runs/domain/query-signature.ts`) derived from the CV's title/mobility/experience-band/contract-type. It checks freshness cache-aside (Redis first, falling back to and repopulating from `FetchRun.querySignature`), takes a short-lived Redis lock before fetching (`fetch-lock.port.ts`/`redis-fetch-lock.adapter.ts`), and triggers `ExecuteFetchRunLifecycleUseCase` only when needed — no route triggers a fetch, and there is no timer. A manual/ops trigger is still available as a CLI script: `yarn run-jobs` (`apps/back/src/composition-root/run-jobs-cli.ts`/`run-jobs.ts`), which calls the same `FetchRunScheduler.triggerRun()` seam (unscoped, for ops use). `POST /api/match` and `GET /api/match/:id` (§4) are implemented — `match/infrastructure/adapters/input/rest/match.controller.ts`, backed by the Redis rate limiter and Redis match-ticket store — and the frontend (`apps/front/src/composables/useMatchFlow.ts`) already calls them for real, no mock data in the request path. **Still outstanding: `GET /api/jobs` and `GET /api/runs`.**

**Scoring is real; embeddings are still stubbed.** `ScoringProviderPort` is bound to `OpenRouterScoringProviderAdapter` (ADR 0020, closing issue #21) — a free-tier interim provider via OpenRouter (native `fetch`, `OPENROUTER_API_KEY` hard-required at boot) chosen after Gemini's free tier proved too rate-limited to serve even a single request in practice. Originally `minimax/minimax-m3:free`; that model was discontinued from OpenRouter's free tier (404s) and the adapter now runs on `nvidia/nemotron-3-super-120b-a12b:free` instead (see ADR 0020's Update). This is explicitly interim: a paid Claude-backed adapter is the intended next step, swappable in behind the same port with no other pipeline changes. `StubEmbeddingProviderAdapter` (ADR 0018) still backs the role-gate similarity check — Anthropic has no embeddings endpoint and Voyage AI (its recommended partner) isn't free, so that stays stubbed with no tracked follow-up yet. `StubScoringProviderAdapter` (ADR 0017) is unbound but left in place as historical record.
...
**Backend structure (domain-first, see ADR 0022):** `apps/back/src/` is organized by bounded context, not by layer — each of `cv/`, `fetch-runs/`, `jobs/`, `match/`, `rate-limiting/`, `scoring/` has its own `domain/`, `application/` (`ports/`, `usecases/`), and `infrastructure/` (`adapters/`) subfolders, so opening the module tells you what it does before what layer you're in. `shared/` holds cross-cutting domain vocabulary used by ≥2 modules (`contract-type.ts`, `french-region.ts`, `raw-job.entity.ts`, the `PORT_TYPES` DI tokens, `LoggerPort`) — not a dumping ground, only things with more than one real owner. `composition-root/` holds the process entrypoints (`main.ts`, `bootstrap.ts`, `config.ts`, the CLI scripts) and the Inversify DI container (`composition-root/container/`, `bind-*.ts` per concern) — the only place concrete infrastructure gets wired together. Cross-module imports use TypeScript path aliases (`@cv/*`, `@fetch-runs/*`, `@jobs/*`, `@match/*`, `@rate-limiting/*`, `@scoring/*`, `@shared/*`, `@composition-root/*`, defined in `apps/back/tsconfig.json`) rather than deep relative paths; within a module, relative imports are still normal. Aliases are resolved natively by `tsx` (dev/CLI), via `moduleNameMapper` in `jest.config.cjs` (tests), and rewritten to relative paths in the compiled output by `tsc-alias` as a `build` post-step (prod).
...
14. **Each app is Screaming Architecture compliant.** Directory structure surfaces the domain (jobs, match, scoring, fetch-runs, rate-limiting...) at the top level, with layers nested inside each module — not framework plumbing at the top level. See ADR 0022 and the "Backend structure" note above.
...
`apps/back/src/composition-root/config.ts` (Zod-validated) is the source of truth for what the backend requires at boot. When a required variable is missing or invalid, the app must log a clear error naming the variable and exit with code 1.
...
| Reintroducing a persisted profile/score model or a public fetch-trigger route | Superseded by ADR 0016/0021 — build `/api/jobs`, `/api/runs` instead; the fetch is triggered from inside the match pipeline, not a route |
...
| Adding an HTTP route to trigger the job pool refresh                          | Fetch is triggered from `CreateMatchRequestUseCase` via `EnsureFreshJobPoolForSignatureUseCase` (ADR 0021), scoped/gated per query signature — no route calls the connectors, and there is no timer |
