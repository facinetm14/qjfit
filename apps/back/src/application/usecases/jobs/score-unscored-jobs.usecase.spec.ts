...
describe("ScoreUnscoredJobsUseCase", () => {
  ...
  it("marks the job as score_failed and logs when the scoring provider fails", async () => {
    const repo = new FakeJobsRepository([buildJob("1")]);
    const scorer: ScoringProviderPort = {
      async score() {
    ...
    const usecase = new ScoreUnscoredJobsUseCase(repo, scorer, scoringRepo, 5, logger);

    await usecase.execute(1);

    expect(repo.failedScores).toEqual(["1"]);
    expect(scoringRepo.saved.length).toBe(0);
    expect(loggedErrors).toHaveLength(1);
    expect(loggedErrors[0]?.message).toBe("Failed to validate score payload");
  });
});
