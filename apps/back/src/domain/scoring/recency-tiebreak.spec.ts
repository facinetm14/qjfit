...
describe("selectRecentCandidates", () => {
  it("sorts by fetchedAt descending (most recent first)", () => {
    const oldest = buildJob("oldest", "2026-01-01T00:00:00.000Z");
    const newest = buildJob("newest", "2026-03-01T00:00:00.000Z");
    const middle = buildJob("middle", "2026-02-01T00:00:00.000Z");
  ...
  it("takes only the top N (default 50)", () => {
    const jobs = Array.from({ length: 60 }, (_, index) =>
      buildJob(`job-${index}`, new Date(2026, 0, index + 1).toISOString()),
    );
  ...
  it("accepts a configurable limit", () => {
    const jobs = [
      buildJob("a", "2026-01-01T00:00:00.000Z"),
      buildJob("b", "2026-01-02T00:00:00.000Z"),
      buildJob("c", "2026-01-03T00:00:00.000Z"),
    ];
